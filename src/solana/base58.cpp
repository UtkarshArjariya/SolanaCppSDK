...
std::string Base58::encode(const std::vector<unsigned char> &input) {
  ...
  for (unsigned char byte : input) {
    ...
    for (int j=0;j<digitslen;j++){carry+=(unsigned int)(digits[j])<<8;digits[j]=(unsigned char)(carry%58);carry/=58;}
...
std::vector<unsigned char> Base58::decode(const std::string &input) {
  ...
  for(char c:input){
    if(c&0x80||ALPHABET_MAP[(int)c]==-1) throw std::invalid_argument("Invalid Base58 character");
    unsigned int carry=ALPHABET_MAP[(int)c];
    for(size_t j=0;j<result.size();j++){carry+=(unsigned int)(result[j])*58;result[j]=(unsigned char)(carry&0xff);carry>>=8;}
